        ...
        if(GPU._modeclocks >= 172)
	{
	  ...
	  if(GPU._lcdon)
	  {
	    if(GPU._bgon)
	    {
	      ...
	      var tilerow = GPU._tilemap[GPU._vram[mapbase+t]+(GPU._bgtilebase?128:0)][y];
	      var pixel;
	      var w=160;
	      do
	      {
		...
		GPU._scrn.data[linebase+3] = pixel[3];
	        x++;
		if(x==8) { t=(t+1)&31; x=0; tilerow=GPU._tilemap[GPU._vram[mapbase+t]+(GPU._bgtilebase?128:0)][y]; }
		linebase+=4;
              } while(--w);
	    }
	    if(GPU._objon)
